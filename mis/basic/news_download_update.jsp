...
try{
	...
	String fk_id = "";	// 20221104 May 
	
	//刪除
	if("D".equals(action)){
		TableRecord fd = app_sm.select(tblfd, fd_id);
		fk_id = fd.getString("fk_id");
		...
		if (app_sm.success()) {
			...
			// 回列表頁 
			out.write(HtmlCoder.getForm("listpage", code + ".jsp?np_no="+fk_id, names, values));
	...
	if("S".equals(action)){
		...
		while (i.hasNext()) {
			...
			if (fi.isFormField()){											//這是用來確定是否為文件屬性 
				...
				if(fieldName.equals("fk_id")) fk_id = fieldvalue.trim();
			}
		}
		...
		app_sm.close();
		// 回排序頁 
		out.write(HtmlCoder.getForm("sortpage", code + "_sort.jsp?np_no="+ fk_id, names, values));
...
// 					TableRecord cp = app_sm.select(tblcp, fk_id);
					/*
