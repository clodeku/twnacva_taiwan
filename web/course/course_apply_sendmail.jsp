...
%>
<%
	String page_code = "course"; 			// 功能識別碼 , 於資料庫做資料識別及模組程式檔名用
	...
	if("".equals(aa_id)) {
		out.println("<script> location='"+page_code+"_apply.jsp'; </script>");
	} else {
		// 利用 JavaMail 元件寄送信件
		...
		subject = SiteSetup.getText("cp.name." + lang)+("tw".equals(lang)?" 報名通知信":" Apply notification letter");
		...
		if(url.indexOf("https://")>-1) {
			content = SSLreturnContent(url + "/web/mail/course_apply.jsp?aa_id=" + aa_id + "&lang=" + lang);
		} else {
			Vector urlcontent = HttpURL.returnContent(url + "/web/mail/course_apply.jsp?aa_id=" + aa_id + "&lang=" + lang); // 信件內容產生的 JSP 檔
		...
		try {
			...
			out.println("<script> alert('"+alert_str.get("apply_success."+lang)+"'); </script>");
			out.println("<script> location='../course/course_apply_in_confirm.jsp?aa_id="+aa.getString("aa_id")+"'; </script>");
		} catch (Exception ex) {
			System.out.println("Project:" + projectName + ", Error info:["+ex+"], File name:web/course/course_apply_sendmail.jsp, Time:["+DateTimeTool.dateTimeString()+"]");
