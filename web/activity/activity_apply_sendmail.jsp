...
<%
	...
	if("".equals(aa_id)) {
		...
		if(url.indexOf("https://")>-1) {
			content = SSLreturnContent(url + "/web/mail/activity_apply.jsp?aa_id=" + aa_id + "&lang=" + lang);
		} else {
			Vector urlcontent = HttpURL.returnContent(url + "/web/mail/activity_apply.jsp?aa_id=" + aa_id + "&lang=" + lang); // 信件內容產生的 JSP 檔
		...
		try {
			...
			out.println("<script> location='../activity/activity_apply_in_confirm.jsp?aa_id="+aa.getString("aa_id")+"'; </script>");
		} catch (Exception ex) {
			System.out.println("Project:" + projectName + ", Error info:["+ex+"], File name:web/activity/activity_apply_sendmail.jsp, Time:["+DateTimeTool.dateTimeString()+"]");
