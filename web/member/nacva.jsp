...
<%
    ...
    // NACVA 收費說明資料
    TableRecord nacva_cp = app_sm.select(tblcp, "cp_code=? and cp_lang=?", new Object[]{"nacva", lang});

    // 款項類別列表
    Vector<TableRecord> dms = app_sm.selectAll(tbldm, "dm_code=? and dm_lang=? and dm_category=?", 
            new Object[] {"nacva_payment", lang, ""}, "dm_showseq ASC, dm_createdate DESC");
    ...
    <script language="JavaScript" type="text/JavaScript">
        function checkform(F) {
            ...
            if(F.mp_name.value == "") {
                ...
                F.mp_join[0].focus();
            } else if(F.rs_nacva_code.value == "") {
                alert("<%=alert_str.get("nacva_code."+lang) %>");
                F.rs_nacva_code.focus();
		...
		<!-- InstanceBeginEditable name="main" -->
            <div class="wrap">
                ...
                <div class="right">
                    ...
                    <div class="white_bg">
                        <br>
                        <strong style="padding-left: 6px; font-size: 15px;"><%=show_str.get("nacva_info."+lang) %>:</strong>
                        <section class="text_area">
                            <%=nacva_cp.getString("cp_content") %>
                        </section>
                    </div>
                    <div class="white_bg">
                        <br>
                    ...
                    <form name="form_nacva" method="post" action="<%=page_code %>_update.jsp?action=nacva" enctype="multipart/form-data" onsubmit="return checkform(this);">
                        <div class="white_bg">
                            <!--表單區-->
                            <div class="form_area">
                                ...
                                <div class="form_list fLType2"><!--一列兩個時class內加fLType2-->
		                            <div class="fL_tit">
		                                <%=field_str.get("nacva_code."+lang) %>:
		                                <div class="required_icon">
		                                    *
		                                </div>
		                            </div>
		                            <div class="fLR_info">
		                                <input type="text" name="rs_nacva_code" id="rs_nacva_code">
		                            </div>
		                        </div>
                                ...
                                <div class="form_list "><!--一列兩個時class內加fLType2-->
                                    <div class="fL_tit">
                                        <%=field_str.get("collection_payment."+lang) %>:
                                        <div class="required_icon">
                                            *
                                        </div>
                                    </div>
                                    ...
                                    <%for(TableRecord dm:dms){ %>
                                        <label class="form_label"><!--調整位置用 class 加 form_label-->
                                            ...
                                            <%=dm.getString("dm_title")+" NTD $"+dm.getString("dm_content")+" "+field_str.get("yuan."+lang) %>
                                        </label>
                                        &nbsp;&nbsp;
                                    <%} %>
                                    </div>
